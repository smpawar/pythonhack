...
_temp_high_value_list = random.sample(ALL_POSSIBLE_ITEMS, NUM_HIGH_VALUE_ITEMS_IN_LIST)
HIGH_VALUE_ITEMS_SET = set(_temp_high_value_list) # O(N) one-time setup

N = len(HIGH_VALUE_ITEMS_SET) # This is our N for complexity calculations
...
def calculate_per_diem(trip_log, employee_tier):
    ...
    for day_index, (item_encountered, deal_potential) in enumerate(trip_log):
        ...
        if item_encountered in HIGH_VALUE_ITEMS_SET: 
